...
| `DEXTOOLS_API_KEY` | Optional Token Chair enrichment | Server-only DEXTools API key for `/api/token-chair-sniffer/market`. Do not use a `NEXT_PUBLIC_` key. |
...
Optional. Enables DEXTools enrichment inside Token Chair Sniffer. The app uses
