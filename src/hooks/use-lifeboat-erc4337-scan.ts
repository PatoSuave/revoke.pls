...
export interface UseLifeboatErc4337ScanResult {
  response: LifeboatErc4337ApiResponse;
...
export function useLifeboatErc4337Scan({
  owner,
  chainId,
  chainName,
  enabled = true,
  ...
  chainName: string;
  enabled?: boolean;
}): UseLifeboatErc4337ScanResult {
  const query = useQuery({
    queryKey: ["lifeboat-erc4337", chainId, owner?.toLowerCase() ?? null],
    enabled: enabled && Boolean(owner),
    staleTime: 30_000,
    gcTime: 5 * 60_000,
    queryFn: async ({ signal }) => {
      if (!owner) throw new Error("owner required");
      return fetchLifeboatErc4337Scan({ owner, chainId, signal });
    },
  });
      ...
      return {
        ...
        errors: [
          query.error instanceof Error
            ? query.error.message
            : "ERC-4337 diagnostic request failed.",
        ],
      };
    }
  ...
  const status: UseLifeboatErc4337ScanResult["status"] =
    !enabled || !owner
      ? "idle"
...
async function fetchLifeboatErc4337Scan({
  owner,
  chainId,
  signal,
}: {
  ...
  const url = new URL("/api/lifeboat/erc4337", window.location.origin);
  ...
  const body = (await response.json()) as LifeboatErc4337ApiResponse;
  if (
    !response.ok &&
    body.status !== "config-missing" &&
    body.status !== "unsupported"
  ) {
    throw new Error(body.errors?.[0] ?? "ERC-4337 diagnostic request failed.");
  }
  return body;
}
