...
export function checkOptimismApprovalApiRateLimit(
  key: string,
  now = Date.now(),
): RateLimitResult {
  ...
  const bucket =
    existing && existing.resetAt > now
      ...
      : {
          count: 0,
          resetAt: now + OPTIMISM_APPROVAL_API_RATE_LIMIT.windowMs,
        };
  ...
  const remaining = Math.max(
    OPTIMISM_APPROVAL_API_RATE_LIMIT.maxRequests - bucket.count,
    0,
  );

  return {
    allowed: bucket.count <= OPTIMISM_APPROVAL_API_RATE_LIMIT.maxRequests,
    limit: OPTIMISM_APPROVAL_API_RATE_LIMIT.maxRequests,
    remaining,
...
export function resetOptimismApprovalApiRateLimitForTests() {
